...
import { ServiceCard, ServiceSection } from '../../common/interfaces';
...
export class ServiceSectionService {
  ...
  async createServiceSection(dto: CreateServiceSectionDto) {
  ...
  async updateServiceSection(id: string, dto: UpdateServiceSectionDto) {
    const [rows] = await this.db.execute<ServiceSection[]>(
      `
        SELECT
          BIN_TO_UUID(id) AS id,
  ...
  async removeServiceSection(id: string) {
    const [rows] = await this.db.execute<ServiceSection[]>(
      `
        SELECT id
        FROM service_section
  ...
  async removeServiceCard(id: string) {
    const [rows] = await this.db.execute<ServiceCard[]>(
      `
        SELECT
          id,
